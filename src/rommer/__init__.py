...
def config_dir():
    ...
    config_dir = (
        pathlib.Path(ROMMER_CONFIG)
        if ROMMER_CONFIG
        else pathlib.Path.home() / ".config" / "rommer"
    )
...
def session():
    ...
    class File(Base):
        ...
        def is_dirty(self, stat=None):
            ...
            return (
                self.size is None
                or self.mtime is None
                or self.size != stat.st_size
                or self.mtime < stat.st_mtime
            )
