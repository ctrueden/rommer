...
class RegexPatterns:
    ...
    def __init__(self):
        ...
        self.alpha: Pattern[str] = re.compile(
            r"\((?:\w*?\s)*Alpha(?:\s\d+)?\)", flags=re.I
        )
        self.beta: Pattern[str] = re.compile(
            r"\((?:\w*?\s)*Beta(?:\s\d+)?\)", flags=re.I
        )
        self.proto: Pattern[str] = re.compile(
            r"\((?:\w*?\s)*Proto(?:type)?(?:\s\d+)?\)", flags=re.I
        )
        self.preprod: Pattern[str] = re.compile(
            r"\((?:Pre-production|Prerelease)\)", flags=re.I
        )
        self.dev: Pattern[str] = re.compile(
            r"\((?:DEV|DEBUG|Debug Build)\)", flags=re.I
        )
        ...
        self.revision: Pattern[str] = re.compile(
            r"\(R[eE][vV](?:[ -][0-9A-Z].*?)?\)", flags=re.I
        )
        ...
        self.ps1_2_id: Pattern[str] = re.compile(
            r"\([LSPT][ABCDEILRSTU][ACEKPTUXZ][ACDHJLMNSX]-\d{5}\)"
        )
        self.ps3_id: Pattern[str] = re.compile(
            r"\([XBM][CLR][AEJKTU][BCDMSTVXZ]-\d{5}\)"
        )
        self.ps3_digital_id: Pattern[str] = re.compile(
            r"\([N][P][EHIJKMNOPQUVWX][A-Z]-\d{5}\)"
        )
        ...
        self.ps_vita_id: Pattern[str] = re.compile(
            r"\([PV][CLS][CAJKS][ABCDEFGHIMSX]-\d{5}\)"
        )
        ...
        self.famicom_disk_system_version: Pattern[str] = re.compile(
            r"\(DV [0-9].*?\)", flags=re.I
        )
        self.hyperscan_version: Pattern[str] = re.compile(r"\(USE[0-9]\)")
        self.nec_mastering_code: Pattern[str] = re.compile(
            r"\((?:(?:F|S)A[A-F][ABTS](?:, )?)+\)"
        )

        # === VIDEO STANDARDS ===
        self.ntsc: Pattern[str] = re.compile(r"(?:-)?\s?\(?\bNTSC\b\)?", flags=re.I)
        self.pal: Pattern[str] = re.compile(
            r"(?:-)?\s?\(?\bPAL\b(?:\s[a-zA-Z]+|\s50[Hh]z)?\)?", flags=re.I
        )
        ...
        # === CATEGORIES ===
        self.bios: Pattern[str] = re.compile(
            r"\[BIOS\]|\(Enhancement Chip\)", flags=re.I
        )
        ...
        self.sample: Pattern[str] = re.compile(
            r"\(Sample(?:\s[0-9]*|\s\d{4}-\d{2}-\d{2})?\)", flags=re.I
        )
        ...
        self.promotional: Pattern[str] = re.compile(
            r"EPK|Press Kit|\(Promo\)", flags=re.I
        )
        ...
        self.not_for_resale: Pattern[str] = re.compile(
            r"\((?:Hibaihin|Not for Resale)\)", flags=re.I
        )
        ...
        self.disc: Pattern[str] = re.compile(
            r"\(Dis[ck|que]?\s*\d+(?:\s*of\s*\d+)?\)", flags=re.I
        )
